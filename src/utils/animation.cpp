...
namespace blunted 
{
	...
	void Animation::Apply(
		...
		bool smooth, 
		float smoothFactor, 
		/*const boost::shared_ptr<Animation> previousAnimation, int smoothFrames, */
	...
	*/
	...
	// /old version
		...
		if (updateSpatial) 
			(*nodeMap.find(nodeAnimations.at(0)->nodeName)).second->RecursiveUpdateSpatialData(e_SpatialDataType_Both);
	}

	void Animation::Shift(int fromFrame, int offset) 
    { 
        // todo: offset does not yet work
		if (offset == 1) 
        {
